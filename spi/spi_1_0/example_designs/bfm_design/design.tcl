proc create_ipi_design { offsetfile design_name } {
	...
	set spi_0 [ create_bd_cell -type ip -vlnv spicer.local:user:spi:1.0 spi_0]
...
set_property -name {xsim.simulate.runtime} -value {100ms} -objects [get_filesets sim_1]
	...
	set offset_file "${bd_path}/spi_tb_include.svh"
	set fp [open $offset_file "w"]
	puts $fp "`ifndef spi_tb_include_vh_"
	puts $fp "`define spi_tb_include_vh_\n"
...
set ip_path [file dirname [file normalize [get_property XML_FILE_NAME [ipx::get_cores spicer.local:user:spi:1.0]]]]
set test_bench_file ${ip_path}/example_designs/bfm_design/spi_tb.sv
...
remove_files -quiet -fileset sim_1 spi_tb_include.vh
